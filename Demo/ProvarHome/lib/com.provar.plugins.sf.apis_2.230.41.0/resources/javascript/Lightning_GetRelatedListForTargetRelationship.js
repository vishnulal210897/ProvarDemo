...
while (actionEle) {
	...
	if(component && component.getAttributeValueProvider() && component.getAttributeValueProvider().getConcreteComponent()) {
		var apiName = component.getAttributeValueProvider().getConcreteComponent().get("v.relatedListId")
					|| component.getAttributeValueProvider().getConcreteComponent().get("v.relatedListConfig.relatedListApiName")//winter 18
					|| component.getAttributeValueProvider().getConcreteComponent().getAttributeValueProvider().getConcreteComponent().get("v.relatedListId")
					;
