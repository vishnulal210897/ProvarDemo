...
while (pathEle) {
	...
	if(component && component.getAttributeValueProvider() && component.getAttributeValueProvider().getConcreteComponent()) {
		...
		} else if (attributeToMatch === 'name') {
			pathStepName = component.getAttributeValueProvider().getConcreteComponent().get("v.name");
		} else if (attributeToMatch === 'localId') {
			pathStepName = component.getAttributeValueProvider().getConcreteComponent().getLocalId();
		}
